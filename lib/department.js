...
const getCourses = department => new Promise((resolve, reject) => {
  const { id, subId } = department;
  request(`http://www.inha.ac.kr/cop/search/curriculum.do?siteId=kr&deptCode=${id}&majorCodeH=${subId}&gubun=1`, (err, httpResponse, body) => {
    if (err) {
      reject(err);
    } else {
...
const getInformation = department => new Promise((resolve, reject) => {
  const { name, id, subId } = department;
  request(`http://www.inha.ac.kr/cop/search/introList.do?siteId=kr&deptCode=${id}&majorCodeH=${subId}`, (err, httpResponse, body) => {
    if (err) {
      reject(err);
    } else {
      const { document } = new JSDOM(body).window;
...
const getProfessors = department => new Promise((resolve, reject) => {
  const { id, subId } = department;
  request(`http://www.inha.ac.kr/cop/search/profList.do?siteId=kr&deptCode=${id}&majorCodeH=${subId}`, (err, httpResponse, body) => {
    if (err) {
      reject(err);
    } else {
      const { document } = new JSDOM(body).window;
...
const getRestrictedCourses = department => new Promise((resolve, reject) => {
  const { id, subId } = department;
  request(`http://www.inha.ac.kr/cop/search/curriculum.do?siteId=kr&deptCode=${id}&majorCodeH=${subId}&gubun=9`, (err, httpResponse, body) => {
    if (err) {
      reject(err);
    } else {
      const { document } = new JSDOM(body).window;
